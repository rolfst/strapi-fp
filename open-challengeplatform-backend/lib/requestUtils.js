...
const defaultResponseSetup = (ctx, customSendFn) => {
  ...
  const sendBad = (ctx) => (err) => {
    if (err instanceof NotAllowedError) return ctx.forbidden(err.message)
...
const sendInternalError = (ctx) => (x) => {
  ctx.log.error(x)
  return ctx.badImplementation(x.message)
}
