...
const _uploadFiles = async (entry, files, { model, source }) => {
  ...
  const doUpload = async (key, files) => {
    ...
    if (model) {
      ...
      return uploadService.uploadToEntity({ id, model: model }, { [field]: files }, source)
    }
  }

  return Promise.all(Object.keys(files).map((key) => doUpload(key, files[key])))
}

/**
 * @param {Promise<Reaction>} entity
 * @param {[]<file} files
 * @param {string} model - the model
 * @param {string} source - the name of the plugin
 */
module.exports = async (entity, files, model, source) => {
  return entity.then(async (entry) => {
    await _uploadFiles(entry, files, { model, source })
